...
class Mtrangle extends Panel implements ActionListener
{
	...
	TextField ta=new TextField(6),tb=new TextField(6),tc=new TextField(6),tr=new TextField(24);
	Button Button=new Button("确定");
	MyDialog dia;
	Mtrangle()
	{
		add(new Label("输入三边长度："));
		add(ta);add(tb);add(tc);
		add(new Label("面积是："));
		add(tr);add(Button);
...
class Win extends Frame implements ActionListener
{
	...
	Win()
	{
		...
		men.add(item1);men.add(item2);
		men2.add(item3);men2.add(item4);
		bar.add(men);
		bar.add(men2);
		setMenuBar(bar);
