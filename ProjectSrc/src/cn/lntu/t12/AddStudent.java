...
public class AddStudent extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e){
		...
		{
			String snumber=JTNumber.getText();//将框中的文本传递给snumber
			...
			try {
				...
				Connection cot=DriverManager.getConnection("jdbc:odbc:student1","","");//打开数据库链接数据源student1
				java.sql.Statement stm=cot.createStatement();//提交查询
				ResultSet rs= stm.executeQuery(sql);//返回结果
				if(rs.next())//查询结果是否存在
				{
					JOptionPane.showMessageDialog(null, "该号已经存在");}//通过showmessagedialog打印信息
					else {
