...
public class ProjectResultPanel extends JPanel{
	private JLabel JLPstates,JLPkeyword,JLPrompt;
	...
	private void initialFrame() {
		...
		JLPrompt= new JLabel("提示:只可删除已申报项目,已审批以及已验收项目不可删除");
		...
		JLPrompt.setBounds(170,550,500,30);
	...
	private void addListener() {
		JBSearch.addActionListener(new ActionListener() {			
			...
			public void actionPerformed(ActionEvent e) {
				...
				try{					
					List<ProjectSearch> psl = pService.projectSearch(Keyword, states , user.getUserName());
					if(psl.size()>=30){
		...
		JBDelete.addActionListener(new ActionListener() {						
			...
			public void actionPerformed(ActionEvent e) {				
				...
				int row = JTproject.getSelectedRow();				
					if(row == -1){					
						JOptionPane.showMessageDialog(getParent(),"请选择要删除的行!");	
						return;
					}else{					
						
						int Pnum = Integer.valueOf((String)tableModel.getValueAt(row, 0));
						int res = pService.projectDelete(Pnum);
						if(res==0){
